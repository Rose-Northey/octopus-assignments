...
export class ParcelTree {
    ...
    recalculateHeightsAndBalancesOfBranch(parcel: Parcel) {
        parcel.height = 1 + Math.max(parcel.left?.height ?? 0, parcel.right?.height ?? 0);
    ...
    rotate(unbalancedParcel: Parcel, childOfInterest: Parcel, isLeft: boolean): void {
    ...
    public locateParcel(desiredHouseNumber: number, currentParcel = this.root): Parcel | undefined {
        if (!currentParcel) {
    ...
    parentReplacesFirstChildWithSecondChild(oldChild: Parcel, newChild: Parcel | undefined) {
